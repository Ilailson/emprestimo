            ...
            <tr v-for="emp in emprestimos" :key="emp.id" :class="['transition-colors', getStatusClass(emp)]">
              ...
              <td class="px-3 py-3">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    v-if="!isPago(emp) && emp.cliente_telefone"
                    @click="abrirWhatsApp(emp)"
                    class="p-1.5 text-green-400 hover:text-green-300 hover:bg-green-500/10 rounded-lg transition-all"
                    title="WhatsApp"
                  >
      ...
      <div v-for="emp in emprestimos" :key="emp.id" :class="['bg-slate-900 border rounded-xl p-4', getStatusClass(emp) === 'bg-red-500/10' ? 'border-red-500/30' : 'border-slate-800']">
        ...
        <div class="flex gap-2 mt-4 pt-3 border-t border-slate-800">
          ...
          <button
            v-if="!isPago(emp) && emp.cliente_telefone"
            @click="abrirWhatsApp(emp)"
            class="p-2.5 text-green-400 hover:text-green-300 hover:bg-green-500/10 rounded-lg transition-all transform hover:scale-110 active:scale-95"
            title="WhatsApp"
          >
...
export default {
  ...
  methods: {
    ...
    abrirWhatsApp(emp) {
      if (!emp.cliente_telefone) return
      let tel = emp.cliente_telefone.replace(/\D/g, '')
